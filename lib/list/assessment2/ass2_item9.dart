...
class Assess2Item9 extends StatefulWidget {
  final String activityCode;

  Assess2Item9({Key? key, required this.activityCode}) : super(key: key);

  @override
  _Assess2Item9State createState() => _Assess2Item9State();
}

class _Assess2Item9State extends State<Assess2Item9> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _itemcode = itemCode.isNotEmpty ? itemCode[8]['ItemCode'] : 'No Code';

          final word = data['Items'] as List<dynamic>;
          _getword = word.isNotEmpty ? itemCode[8]['Word'] : 'No Word';
  ...
  Future<void> startRecording() async {
    if (await Permission.microphone.isGranted) {
      ...
      _pathToAudio =
          path.join(directory.path, 'temp_${widget.activityCode}_item9.wav');
  ...
  Future<void> sendAudio() async {
    ...
    try {
      ...
      await prefs.setString('User9', _pathToAudio); // Save the file path
      await prefs.setString('ItemCode9', _itemcode); // Save the item code
