...
class Assess2Item6 extends StatefulWidget {
  final String activityCode;

  Assess2Item6({Key? key, required this.activityCode}) : super(key: key);

  @override
  _Assess2Item6State createState() => _Assess2Item6State();
}

class _Assess2Item6State extends State<Assess2Item6> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _itemcode = itemCode.isNotEmpty ? itemCode[5]['ItemCode'] : 'No Code';

          final word = data['Items'] as List<dynamic>;
          _getword = word.isNotEmpty ? itemCode[5]['Word'] : 'No Word';
  ...
  Future<void> startRecording() async {
    if (await Permission.microphone.isGranted) {
      ...
      _pathToAudio =
          path.join(directory.path, 'temp_${widget.activityCode}_item6.wav');
  ...
  Future<void> sendAudio() async {
    ...
    try {
      ...
      await prefs.setString('User6', _pathToAudio); // Save the file path
      await prefs.setString('ItemCode6', _itemcode); // Save the item code
