...
class Assess2Item8 extends StatefulWidget {
  final String activityCode;

  Assess2Item8({Key? key, required this.activityCode}) : super(key: key);

  @override
  _Assess2Item8State createState() => _Assess2Item8State();
}

class _Assess2Item8State extends State<Assess2Item8> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _itemcode = itemCode.isNotEmpty ? itemCode[7]['ItemCode'] : 'No Code';

          final word = data['Items'] as List<dynamic>;
          _getword = word.isNotEmpty ? itemCode[7]['Word'] : 'No Word';
  ...
  Future<void> startRecording() async {
    if (await Permission.microphone.isGranted) {
      ...
      _pathToAudio =
          path.join(directory.path, 'temp_${widget.activityCode}_item8.wav');
  ...
  Future<void> sendAudio() async {
    ...
    try {
      ...
      await prefs.setString('User8', _pathToAudio); // Save the file path
      await prefs.setString('ItemCode8', _itemcode); // Save the item code
