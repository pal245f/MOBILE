...
class Assess2Item7 extends StatefulWidget {
  final String activityCode;

  Assess2Item7({Key? key, required this.activityCode}) : super(key: key);

  @override
  _Assess2Item7State createState() => _Assess2Item7State();
}

class _Assess2Item7State extends State<Assess2Item7> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _itemcode = itemCode.isNotEmpty ? itemCode[6]['ItemCode'] : 'No Code';

          final word = data['Items'] as List<dynamic>;
          _getword = word.isNotEmpty ? itemCode[6]['Word'] : 'No Word';
  ...
  Future<void> startRecording() async {
    if (await Permission.microphone.isGranted) {
      ...
      _pathToAudio =
          path.join(directory.path, 'temp_${widget.activityCode}_item7.wav');
  ...
  Future<void> sendAudio() async {
    ...
    try {
      ...
      await prefs.setString('User7', _pathToAudio); // Save the file path
      await prefs.setString('ItemCode7', _itemcode); // Save the item code
