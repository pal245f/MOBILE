...
class Assess2Item10 extends StatefulWidget {
  final String activityCode;

  Assess2Item10({Key? key, required this.activityCode}) : super(key: key);

  @override
  _Assess2Item10State createState() => _Assess2Item10State();
  
}

class _Assess2Item10State extends State<Assess2Item10> {
  ...
  Future<void> stopRecording() async {
    ...
    await prefs.setString(
        'User10', _pathToAudio); // Save the latest audio file path
  }

  Future<void> sendAudio(UserProvider userProvider) async {
    final userProvider = Provider.of<UserProvider>(context, listen: false); // Get the current user with lrn
    String lrn = userProvider.user?.lrn ?? ''; // Access the LRN
    try {
      setState(() {
      ...
      print("Activity ${widget.activityCode} is marked as completed.");
        ...
        // Extract UserInputId from the response, with additional error handling
        if (jsonData.containsKey('UserInputId') &&
          ...
          return; // Exit early if UserInputId is missing
        }

        setState(() {
          _isAudioSent = true; // Mark audio as sent
        });
        showError('Audio uploaded successfully!');

      // Set the completion flag in SharedPreferences

        // Navigate to ResultScreen2 with UserInputId
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => ResultScreen2(
              activityCode: widget.activityCode,
              userInputId: userInputId, // Pass UserInputId to ResultScreen2
            ),
          ),
        );

        await clearLocalStorage(); // Clear all items from local storage
      } else {
        final responseBody = await response.stream.bytesToString();
        showError(
