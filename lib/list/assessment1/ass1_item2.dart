...
class Assess1Item2 extends StatefulWidget {
  ...
  Assess1Item2({Key? key, required this.activityCode}) : super(key: key);

  @override
  _Assess1Item2State createState() => _Assess1Item2State();
}
...
class _Assess1Item2State extends State<Assess1Item2> {
  ...
  Future<void> initialize() async {
    ...
    _pathToAudio =
        path.join(directory.path, 'temp_${widget.activityCode}_item2.wav');
  ...
  Future<void> fetchActivityData() async {
    ...
    print('Code2: ${widget.activityCode}');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _itemcode = itemCode.isNotEmpty ? itemCode[1]['ItemCode'] : 'No Code';

          final word = data['Items'] as List<dynamic>;
          _getword = word.isNotEmpty ? itemCode[1]['Word'] : 'No Word';
  ...
  Future<void> sendAudio() async {
    ...
    try {
      ...
      await prefs.setString('User2', _pathToAudio);
      await prefs.setString('ItemCode2', _itemcode);
      _isAudioSent = true;
